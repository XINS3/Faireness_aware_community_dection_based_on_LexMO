...
def redFairness_louvain_communities(
    ...
    d = redFairness_louvain_partitions(G, weight, resolution, threshold_mod,threshold_fmod, seed, node_attributes=node_attributes)
    q = deque(d, maxlen=1)
    return q.pop()
...
def redFairness_louvain_partitions(
    ...
    fmod,communityFairModularityist,fairModPercList,redModularityList,blueModularityList = modularityFairnessMetric(G, partition,node_attributes, resolution=resolution, weight=weight)
    ...
    partition, inner_partition, improvement = _one_level(
        graph,iterationNum,mod,fmod,communityFairModularityist,communityModularityist, m, partition,com2node, resolution, is_directed, seed, node_attributes=node_attributes)
    improvement = True
    iterationNum = 1
    ...
    while improvement:
        ...
        partition, inner_partition, improvement = _one_level(
            graph,iterationNum,new_mod,new_fmod,communityFairModularityist,communityModularityist, m, partition,com2node, resolution, is_directed, seed, node_attributes=node_attributes)
        iterationNum += 1
                ...
                if iterationNum==0:
                    if node_attributes[u] == 1:
                        remove_cost_R = -((weights2comInter[best_com])) / (2*m) + resolution * (
                                (Stot_r[best_com]) * degree) / ((2 * m)**2)
                
                    else:
                
                        remove_cost_R = -((2*weights2comR[best_com]+weights2comInter[best_com])) / (2*m) + resolution * (
                                (Stot[best_com]+Stot_r[best_com]) * degree) / ((2 * m)**2)
                else:
                    remove_cost_R = -((2*weights2comR[best_com]+weights2comInter[best_com])) / (2*m) + resolution * (
                                Stot[best_com]*degree_r+Stot_r[best_com] * degree) / ((2 * m)**2)
                ...
                if is_directed:
                    ...
                    if iterationNum==0:
                        if node_attributes[u] == 1:
                            gain_r = (
                                (wtInter) / (2*m)
                                - resolution * ((Stot_r[nbr_com]) * degree) / ((2 * m)**2)
                            )
                            
                        else:
                            gain_r = (
                               ((2*wtR)+wtInter) / (2*m)
                                - resolution * ((Stot[nbr_com]+Stot_r[nbr_com]) * degree) / ((2 * m)**2)
                            )
                        ...
                        gain_r = (
                         ((2*wtR)+wtInter) / (2*m)
                        - resolution * (Stot[nbr_com]*degree_r+Stot_r[nbr_com] * degree) / ((2 * m)**2)
                        )
