...
def Ldiversity_louvain_communities(
    G, weight="weight", resolution=1, threshold_mod=0.0000001,threshold_fmod=0.001, seed=None, node_attributes={},
):
    

    d = Ldiversity_louvain_partitions(G, weight, resolution, threshold_mod,threshold_fmod, seed, node_attributes=node_attributes)
    q = deque(d, maxlen=1)
    return q.pop()


@py_random_state("seed")
def Ldiversity_louvain_partitions(
    G, weight="weight", resolution=1, threshold_mod=0.0000001,threshold_fmod=0.001, seed=None, node_attributes={},
):
    ...
    if G.is_multigraph():
        ...
        for u,v in graph.edges():
            if node_attributes[u] == 0 and node_attributes[v] == 0: # If either node is red
                graph[u][v]['r_weight'] = 1
                graph[u][v]['b_weight'] = 0
                graph[u][v]['inter_weight'] = 0
                
            elif node_attributes[u] == 1 and node_attributes[v] == 1: # If either node is blue
            ...
            if node_attributes[u] == 1 and node_attributes[v] == 0: # red
                graph.nodes[u]['red_weight'] +=1        
                graph.nodes[v]['blue_weight'] +=1
            if node_attributes[u] == 0 and node_attributes[v] == 1: # blue
    ...
    diversityModularity,diversityModularityList, = LDiversityFairnessMetric(G, partition, G_attribute = node_attributes, resolution=resolution, weight=weight) 
    ...
    deg_sum = sum(wt for u, v, wt in graph.edges( data="inter_weight", default=1))
    mInter = deg_sum
    ...
    partition, inner_partition, improvement = _one_level(
        graph,iterationNum,mod,diversityModularityList,communityModularityist, m,mInter, partition,nodesList, resolution, is_directed, seed, node_attributes=node_attributes)
    improvement = True
    iterationNum = 1
    while improvement:
        ...
        diversityModularity,new_diversityModularityList, = LDiversityFairnessMetric(G, partition, G_attribute = node_attributes, resolution=resolution, weight=weight) 
        



    
        check = sum(new_diversityModularityList) - sum(diversityModularityList)<=0
        ...
        new_diversityModularityList = new_diversityModularityList
        ...
        partition, inner_partition, improvement = _one_level(
            graph,iterationNum,new_mod,new_diversityModularityList,communityModularityist, m,mInter, partition,nodesList, resolution, is_directed, seed, node_attributes=node_attributes)
        iterationNum += 1
...
def _one_level(G,iterationNum,orig_mod,diversityModularityList,modList, m,mInter, partition,nodesList, resolution=1, is_directed=False, seed=None, node_attributes={}):
    ...
    while nb_moves > 0:
        ...
        for u in rand_nodes:
            ...
            if is_directed:
                ...
                remove_cost_inter = -weights2comInter[best_com] / m + resolution * ((Stot_r[best_com] * degree_b)+(Stot_b[best_com] * degree_r)) / (2 * mInter*m)
            ...
            for nbr_com, wt in weights2com.items():
                if is_directed:
                    ...
                    gain = (
                        wt / mInter
                        - resolution * (Stot[nbr_com] * degree) / (2 * mInter*m)
                    )

                    

                    
                    gain_rb = (wtInter / m) - resolution * ((Stot_r[nbr_com] * degree_b)+(Stot_b[nbr_com] * degree_r)) / (2 * mInter*m)
                ...
                fair_Inter_before = diversityModularity_before_nbr_com + diversityModularity_before_best_com
                fair_Inter_after = diversityModularity_after_nbr_com +diversityModularity_after_best_com
