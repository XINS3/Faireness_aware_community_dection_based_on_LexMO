...
def LRedFairness_louvain_communities(
    ...
    d = LRedFairness_louvain_partitions(G, weight, resolution, threshold_mod,threshold_fmod, seed, node_attributes=node_attributes)
    q = deque(d, maxlen=1)
    return q.pop()
...
def LRedFairness_louvain_partitions(
    ...
    m2 = graph.size(weight = 'inter_weight')
    ...
    partition, inner_partition, improvement = _one_level(
        graph,iterationNum,mod,fmod,communityFairModularityist,communityModularityist, m,m1,m2, partition,com2node, resolution, is_directed, seed, node_attributes=node_attributes)
    improvement = True
    iterationNum = 1
    ...
    while improvement:
        ...
        partition, inner_partition, improvement = _one_level(
            graph,iterationNum,new_mod,new_fmod,communityFairModularityist,communityModularityist, m,m1,m2, partition,com2node, resolution, is_directed, seed, node_attributes=node_attributes)
        iterationNum += 1
    ...
    while nb_moves > 0:
        ...
        for u in rand_nodes:
            ...
            if is_directed:
                ...
                if iterationNum==0:
                    if node_attributes[u] == 1:
                        ...
                        remove_cost_R = -((weights2comInter[best_com])) / (2*m) + resolution * (
                                (Stot_blue_red_nodes[best_com]) * degree_b) / ((2 * m)**2)
                
                    else:
                
                        remove_cost_R = -((2*weights2comR[best_com]+weights2comInter[best_com])) / (2*m) + resolution * (
                                (Stot_blue_red_nodes[best_com]) * degree_blue_red_node) / (2 * m*m2) - (Stot_r[best_com]*degree_r)/(4*m*m1)
                else:
                    remove_cost_R = -((2*weights2comR[best_com]+weights2comInter[best_com])) / (2*m) + resolution * (
                                Stot_red_blue_nodes[best_com]*degree_red_blue_node+Stot_blue_red_nodes[best_com] * degree_blue_red_node) / (2 * m*m2) - (Stot_r[best_com]*degree_r)/(4*m*m1)
            ...
            for nbr_com, wt in weights2com.items():
                ...
                if is_directed:
                    ...
                    if iterationNum==0:
                        if node_attributes[u] == 1:
                            gain_r = (
                                (wtInter) / (2*m)
                                - resolution * ((Stot_blue_red_nodes[nbr_com]) * degree_b) / (2 * m*m2)
                            )
                            ...
                            gain_r = (
                               ((2*wtR)+wtInter) / (2*m)
                                - resolution * (
                                    (Stot_blue_red_nodes[nbr_com]) * degree_blue_red_node) / (2 * m*m2) - (Stot_r[nbr_com]*degree_r)/(4*m*m1)
                                )
                        ...
                        gain_r = (
                         ((2*wtR)+wtInter) / (2*m)
                        - resolution * (Stot_blue_red_nodes[nbr_com]*degree_blue_red_node+Stot_red_blue_nodes[nbr_com] * degree_red_blue_node) / (2 * m*m2) - (Stot_r[nbr_com]*degree_r)/(4*m*m1)
                    ...
                    gain_b = (
                         wtB / m
                        - resolution * (Stot[nbr_com] * degree_b) / (2 * m**2)
                    )
                ...
                check_in = mod_gain>best_gain and redmod_gain>=best_red_gain
